...
func formatMasterData(accounts []bson.M, journalBooks []bson.M, creditors []bson.M, documentTemplates []bson.M) string {
...
`, string(accountsJSON))
	...
	return fmt.Sprintf(`%sข้อมูลหลัก - สมุดรายวัน:
%s

ข้อมูลหลัก - เจ้าหนี้:
%s%s`, accountsSection, string(journalBooksJSON), string(creditorsJSON), templatesSection)
}
...
4. **เจ้าหนี้**: ต้องเลือกจากรายการเจ้าหนี้ที่กำหนดให้ หรือใช้ "Unknown Vendor" ถ้าไม่เกี่ยวข้องจริง และตั้ง requires_review = true
...
🏢 การจับคู่ผู้ขาย/เจ้าหนี้:
- ค้นหารายการเจ้าหนี้ที่ตรงกับชื่อผู้ขาย
- ใช้การจับคู่แบบคลุมเครือ (ชื่อบางส่วน, การสะกดคล้ายกัน)
- ถ้าหาไม่เจอ → ใช้ "AR0005 - บิลเงินสด" (ค่าเริ่มต้น)
...
func BuildMultiImageAccountingPrompt(allResultsJSON string, accounts []bson.M, journalBooks []bson.M, creditors []bson.M, documentTemplates []bson.M) string {
	masterData := formatMasterData(accounts, journalBooks, creditors, documentTemplates)
