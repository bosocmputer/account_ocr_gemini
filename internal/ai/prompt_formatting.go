...
3. ถ้า**ชื่อบริษัทหลักตรงกัน ≥ 70%%** → นี่คือ**เอกสารภายใน**หรือ**เราเป็นผู้ขาย**:
   ✅ ตัวอย่างที่ถูกต้อง:
      - vendor_name = "[บริษัทเรา]" + names[0].name = "[บริษัทเรา]" → "[ชื่อหลัก]" ตรงกัน 100%% ✓
      - vendor_name = "[บริษัทเราชื่ออื่น]" + names[0].name = "[บริษัทเราชื่อหลัก]" → "[ชื่อหลัก]" ตรงกัน 100%% ✓
      - vendor_name = "[Company Name]" + names[1].name = "[Company Name]" → "[Name]" ตรงกัน 100%% ✓
   
   ❌ ตัวอย่างที่ผิด (ต้อง match กับ Creditors แทน):
      - vendor_name = "[บริษัทอื่น]" + names[0].name = "[บริษัทเรา]" → "[ชื่ออื่น]" ≠ "[ชื่อเรา]" ✗
      - vendor_name = "[Company A]" + names[0].name = "[Company B]" → "A" ≠ "B" ✗
...
2. เปรียบเทียบกับชื่อบริษัทเรา (ดูใน names[].name ในบริบทธุรกิจข้างบน):
   - ชื่อบริษัทเราอยู่ใน names array โดยดูที่ field "name" (เช่น names[0].name = "[ชื่อบริษัทเรา]")
...
3. เทคนิคการจับคู่ (ใช้ Fuzzy Matching):
   ...
   - ⚠️ **ตัวอย่าง:** "[ชื่อที่มีตัวสะกดต่าง]" ควร match กับ "[ชื่อเดียวกันแต่สะกดต่าง]" (ต่างแค่ตัวสะกด)
